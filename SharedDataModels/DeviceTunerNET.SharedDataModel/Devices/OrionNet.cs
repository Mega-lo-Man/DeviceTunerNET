...
{
    public static class OrionNet
    {
        public enum PacketIndexes
        {
            address = 0,
            packetLength = 1,
            commandCounter = 2
        ...
        private static bool IsCrcValid(IEnumerable<byte> packetWithCrc)
        {
        ...
        private static bool IsReceivePacketComplete(IEnumerable<byte> packet)
        {
            if (packet.Count() < 2)
                return false;
